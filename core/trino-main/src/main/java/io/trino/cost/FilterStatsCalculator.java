...
public class FilterStatsCalculator
{
            ...
            extends AstVisitor<PlanNodeStatsEstimate, Void>
    {
        ...
        {
            PlanNodeStatsEstimate andEstimate = process(terms.get(0));
            for (int i = 1; i < terms.size(); i++) {
                andEstimate = new FilterExpressionStatsCalculatingVisitor(andEstimate, session, types).process(terms.get(i));
            ...
            PlanNodeStatsEstimate summed = process(terms.get(0));
            for (int i = 1; i < terms.size(); i++) {
                summed = addStatsAndSumDistinctValues(summed, process(terms.get(i)));
            }

            if (summed.isOutputRowCountUnknown()) {
                return PlanNodeStatsEstimate.unknown();
            }

            return capStats(
                    subtractSubsetStats(summed, andEstimate),
                    input);
        }
